...
SAME score definition (faithfulness = SUPPORTED/(SUPPORTED+CONTRADICTED) = factual PRECISION, which is
...
false, faithfulness must be 0.0 and uncovered must be 0.0, regardless of the answer's opening.

Compute faithfulness as a PURE PROPORTION: SUPPORTED / (SUPPORTED + CONTRADICTED), weighting every claim
equally with no severity weighting. For example, a biography with one supported and one contradicted claim
scores 1 / (1 + 1) = 0.5. Faithfulness is computed ONLY over SUPPORTED + CONTRADICTED claims; uncovered is
computed over ALL substantive claims. These are different denominators — do not normalise them together. If
there are NO supported and NO contradicted claims (every substantive claim is uncovered), faithfulness is
...
{"faithfulness": 0.6666666666666666, "uncovered": 0.0, "coherent": true}
