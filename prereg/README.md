...
Each file states a hypothesis, the population it will be tested on, the numeric bar it has to clear,
and the rule for deciding the outcome. Each was committed before the run it describes, so the commit
timestamp shows that the prediction pre-dates the result. Use `git log --follow -- prereg/<file>` to
see when one was written; the files carry descriptive names that were applied after they were
created, so a plain `git log` reports the renaming rather than the writing.

Most of these record a negative, a null or a tie, and they are kept for that reason. The report
argues that no fixed way of combining token probabilities works across long-form tasks, and that
argument rests on knowing which nearby designs were tried and did not work. A pre-registration that
is removed once it fails is worth nothing.

The `Recorded outcome` column quotes each file's own outcome line.
...
### The benchmark and its populations

How the evaluation targets, training pools and response populations were defined, and the validity checks applied to them before any uncertainty score was computed.
...
### CAWSA and its constraint

The method reported in the write-up: the shrinkage constraint, the alternatives to it, and the attempts to reach the same effect another way.
...
Whether any fixed or simply routed rule for combining token probabilities works across targets. These are the registrations behind the conclusion that none does.
...
Whether more flexible or more directed pooling of internal representations transfers. Every one of these is a recorded negative.
...
### Published comparators and combinations

Reproductions of methods from the literature, and the two ways CAWSA was combined with a hidden-state probe.
