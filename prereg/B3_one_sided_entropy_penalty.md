# PRE-REGISTRATION — B.3: the one-sided entropy penalty
...
target carries no information. `real ≈ shuffled` on every cell, across the paper's λ range, Joe's
...
⚠️ **A penalty that binds everywhere is just the two-sided entropy control we already ran and found
...
## ⚠️ Scope — narrower than B.1/B.2 assumed
...
2. **xsum ID: no-op.** Its attention is already broad, so the hinge should not bind and PRR should not
   move.
3. ⚠️ **A uniform effect across both is SUSPICIOUS** and must be checked for an artefact before being
...
**unimproved** attention probe, which is exactly the weaker story Joe warned about at the 31 July
