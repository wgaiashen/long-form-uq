# PRE-REGISTRATION — 0.1: does attention flattening predict performance drop?
...
Joe's explicit request at the 31 July meeting:
...
## ⚠️ Prior evidence already visible — the hypothesis is ALREADY broken at dataset level

From `STOCKTAKE_post24July.md:688` (E1 dump, job `3468720`, 4 datasets) joined to `pdl_master`
(`armA(attention)`, ID vs the mean of the four OOD rungs). **All four points verified this session:**
...
## ⚠️ If the dataset-level correlation comes back STRONG, treat it as a suspect
...
- **Weak or wrong-signed** → entropy is **not** the switching signal. Phase 3 gates on **length**, and
  this is reported as a clean negative answering Joe's question. That is a result, not a failure.
...
per-dataset scatter plot Joe asked for, and the within/across decomposition from prediction 3. State
