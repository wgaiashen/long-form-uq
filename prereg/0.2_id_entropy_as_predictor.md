# PRE-REGISTRATION — 0.2: does the ID attention entropy alone predict the OOD drop?
...
## ⚠️ READ THIS FIRST — THIS ONE IS NOT BLIND, AND SAYING SO IS THE POINT
...
## ⚠️ PSEUDO-REPLICATION — the honest n is 8, not 32
...
- **The secondary U-shape test is declared HERE, not invented after:** correlate `|ne_ID − median(ne_ID)|`
  against the drop. ⚠️ It is a second test on the same eight points, so it carries a multiple-comparisons
...
- ⚠️ **A favourable result here is a SUSPECT**, because the data were already visible. If it clears the
