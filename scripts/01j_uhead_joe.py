"""GPU step: the Table-1 UHead baseline reproduced by running JOE'S ACTUAL `luh` code.
...
split, and writes the `uhead` / `uhead_v2` column 04_eval reads. See src/luq/uhead_joe.py for the
faithfulness argument and the documented deviations.

    python scripts/01j_uhead_joe.py --model meta-llama/Meta-Llama-3.1-8B --dataset sciq --variant v1

Defaults: fp32 + eager base (matches Joe, and eager returns the attentions his extractor needs) and
...
from luq import cache, generate, results, uhead_joe  # noqa: E402
...
def main():
    ...
    ap.add_argument("--variant", default="v1", choices=list(uhead_joe.VARIANTS),
    ...
    ap.add_argument("--precision", default="fp16", choices=["fp16", "fp32"],
                    help="head training precision: fp16 = Joe's fp16=True (autocast+GradScaler)")
    # Base forward: fp32 + eager matches Joe (he passes no torch_dtype, --attn eager) and eager is
    ...
    head = uhead_joe.build_uhead(model, args.variant)
    ...
    for i, r in enumerate(records):
        li, lo, _ = uhead_joe.build_llm_io(model, tok, r, max_seq=args.max_seq)
        X, om, am = uhead_joe.extract_features(head, li, lo)
    ...
    unc = uhead_joe.predict(head, [feats[i] for i in idx_te], [oms[i] for i in idx_te],
    ...
    path = cache.save_scores(unc, cfg.cache_dir, key, method=args.name, layer=-1,
                             ...
                             source="joe_luh", rec_mtime=rec_file.stat().st_mtime)
